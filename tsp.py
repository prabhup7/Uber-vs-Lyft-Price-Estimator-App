...
def distance(temp, locations):
    """locations: list of locations (source and intermediate locations)
		returs: index of the nearest neighbour
	"""
    for x in range(0, len(locations)):
        ...
        a = math.sin(dlat / 2) * math.sin(dlat / 2) + math.cos(math.radians(float(temp["lat"]))) * math.cos(
            math.radians(float(locations[x]["lat"]))) * math.sin(dlon / 2) * math.sin(dlon / 2)
